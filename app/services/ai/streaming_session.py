...
class StreamingInterviewSession:
    ...
    async def initialize_session(self, resume_text: str, jd_text: str, interview_type: str = "technical", role: str = "", company: str = "", duration: int = 0, candidate_name: str = ""):
        self.interview_type = interview_type
        self.role = role
        self.company = company
        self.duration_limit = duration
        self.start_time = time.time()
        self.context_summary = await self.client.summarize_context(resume_text, jd_text, interview_type, role, company, candidate_name)
        self.ended = False
    ...
    async def stream_response(self, user_input: str = None):
        """
        Generates a structured response (Evaluation + Question) and yields dictionaries containing chunks or metadata.
        """
        if user_input:
        ...
        if evaluation.decision.action == Action.END:
            ...
            is_coding = False # No coding on end
